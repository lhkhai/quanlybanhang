...
class CustomerController extends Controller
{
    ...
    {
        $customer = customer::paginate(10);
        return view('/customer.test')->with(['dataview1'=>$customer]);  
    }

    public function getData_test($rowperpage)
    {
        $customer = customer::paginate($rowperpage);
        return view('/customer.test')->with(['dataview1'=>$customer,'rowperpage'=>$rowperpage]);
    }
    ...
    public function search_test(Request $request)
    {
        ...
        {
        $customer = customer::paginate(10);
        return redirect('/customer.test')->with(['dataview1'=>$customer]); 
        }
        else {
         if($request->has('input_search_makh'))
         {
        ...
        $dataview = $query->paginate(10);  
        return view('/customer.test')->with(['dataview1'=>$dataview]);
        }
    }    
