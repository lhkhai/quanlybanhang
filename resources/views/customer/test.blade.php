...
<div class='div_search'> 
  <form class='form_search' method='get' action='{{asset("test/search")}}'> 
 
    
  <input type='text'  class="form-control" placeholder='Nhập mã KH' value="{{ request('input_search_makh') }}"  name='input_search_makh' id='input_search_makh' />
  ...
  <table class="table table-bordered" id="view_table_supplier">
    ...
    <tbody id = 'list_ncc'> 
          @if(isset($dataview1))   
           ...
           <?php $stt=($dataview1->currentPage()-1)* $dataview1->perPage();  ?>
            @foreach($dataview1 as $key=>$value)
...
<div>@isset($dataview1)
    {{$dataview1->appends(request()->all())->links()}}
    @endif
